...
def clicked(seed=-1, *file_paths):
    ...
    # TODO expand content to 15 seconds

    content_shape = content_spectrum.shape
    content_avg = content_spectrum.mean(axis=0)
    content_spectrum = np.expand_dims(content_avg, 0).repeat(content_shape[0], axis=0)
    ...
    for style_path in non_none_paths:
        ...
        m_style_spectrum, _, _ = read_audio_spectrum(style_path)
        if style_spectrum is None:
            style_spectrum = m_style_spectrum
        else:
            style_spectrum = np.concatenate((style_spectrum, m_style_spectrum), axis=1)
